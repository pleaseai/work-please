...
describe('formatInitError', () => {
  ...
  it('formats init_write_failed with path and cause', () => {
    const msg = formatInitError({ code: 'init_write_failed', path: '/tmp/WORKFLOW.md', cause: 'EACCES' })
    expect(msg).toContain('Failed to write')
    expect(msg).toContain('/tmp/WORKFLOW.md')
    expect(msg).toContain('EACCES')
  })

  it('formats init_write_failed with project number hint', () => {
    const msg = formatInitError({
      code: 'init_write_failed',
      path: '/tmp/WORKFLOW.md',
      cause: 'ENOSPC',
      projectNumber: 42,
    })
    expect(msg).toContain('project #42')
    expect(msg).toContain('already created')
  })
})
