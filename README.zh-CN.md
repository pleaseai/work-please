...
Please Work 将问题追踪器中的任务转化为隔离的自主实现运行 —— 管理工作而非监督编码代理。

> **警告**：Please Work 是一个工程预览版，仅适用于受信任的环境。
...
Please Work 是一个长期运行的 TypeScript 服务，它：
...
# 使用当前目录的 WORKFLOW.md 运行 Please Work
bunx please-work
...
`WORKFLOW.md` 是 Please Work 运行时行为的唯一事实来源。它将 YAML front matter
...
workspace:
  root: ~/please-work_workspaces        # 可选: 默认 <tmpdir>/please-work_workspaces
...
claude:
  ...
  settings:
    attribution:
      commit: "🙏 Generated with [Please Work](https://github.com/pleaseai/work)"  # 可选: 附加到 git 提交消息。默认为 Please Work 链接。
      pr: "🙏 Generated with [Please Work](https://github.com/pleaseai/work)"      # 可选: 附加到 PR 描述。默认为 Please Work 链接。
...
Please Work 在启动时验证 GitHub App 配置：
...
Please Work 自主运行 Claude Code。部署前请了解相关的信任影响。
...
- Please Work 是基于 OpenAI 的 [Symphony 规范](vendor/symphony/SPEC.md)（Apache 2.0）的 TypeScript 实现。
